...
Rv =  [0.4;0.4;0.4;0.4;0.4;0.4;0.4;0.4;0.4;0.4];
%
rv=[];
rv=[0;cv(1)-Rv(1)-0.0001];
for k=1:length(cv)-1
    rv=[rv;linspace(cv(k)-Rv(k)+0.02,cv(k)+Rv(k)-0.02,10)'];
    rv=[rv;linspace(cv(k)+Rv(k)+0.02,cv(k+1)-Rv(k+1)-0.02,10)'];
end
rv=[rv;linspace(cv(end)-Rv(end)+0.01,cv(end)+Rv(end)-0.01,10)'];
rv=[rv;linspace(cv(end)+Rv(end)+0.01,12,10)'];
rv=[rv;linspace(12,14,5)'];
alpha   =  0;
z = alpha;
%%
for kk=1:length(rv)
