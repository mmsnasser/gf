...
%
av = [3/3;5/3;9/3; 11/3;21/3;23/3;27/3;29/3];
bv = [4/3;6/3;10/3;12/3;22/3;24/3;28/3;30/3];
%
rv=[];
rv=[0;av(1)-0.0001];
for k=1:length(av)-1
    rv=[rv;linspace(av(k)+0.01,bv(k)-0.001,10)'];
    rv=[rv;linspace(bv(k)+0.01,av(k+1)-0.001,10)'];
end
rv=[rv;linspace(av(end)+0.01,bv(end)-0.001,10)'];
rv=[rv;linspace(bv(end)+0.01,12,10)'];
%%
for kk=1:length(rv)
    r = rv(kk);
    r
