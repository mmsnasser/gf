...
bv=Ev(2:2:end);
%
nvs = [20;10;10;5;5;2;2];
nvg = [20;20;20;10;10;5;4];
%
rv=[];
rv=[0;av(1)-0.0001];
for k=1:length(av)-1
    rv=[rv;linspace(av(k)+0.01,bv(k)-0.001,nvs(L+1))'];
    rv=[rv;linspace(bv(k)+0.01,av(k+1)-0.001,nvg(L+1))'];
end
rv=[rv;linspace(av(end)+0.01,bv(end)-0.001,nvs(L+1))'];
rv=[rv;linspace(bv(end)+0.01,2.2,nvs(L+1))'];
%%
for kk=1:length(rv)
    r = rv(kk);
    r
