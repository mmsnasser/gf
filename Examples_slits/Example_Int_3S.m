...
rv=[0;av(1)-0.0001];
for k=1:length(av)-1
    rv=[rv;linspace(av(k)+0.01,bv(k)-0.001,30)'];
    rv=[rv;linspace(bv(k)+0.01,av(k+1)-0.001,30)'];
end
rv=[rv;linspace(av(end)+0.01,bv(end)-0.001,30)'];
rv=[rv;linspace(bv(end)+0.01,10,20)'];
%%
for kk=1:length(rv)
    r = rv(kk);
    r
